...
let rec runRollDice(gamestate: GameState): GameState =
    ...
    | "1" when gamestate.RollCount > 0 -> 
        printfn "You decided to reroll. Please enter the indices of the dice you want to keep, separated by spaces:"
        let keepIndices = inputToIntList()
        if List.isEmpty keepIndices then
            printfn "No dice kept. Please try again."
            runRollDice gamestate
        else if isValidReroll gamestate.Dice keepIndices then 
            let newDice = rerollDice gamestate.Dice keepIndices
            showDice newDice
